            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap items-center gap-3 bg-white p-3 rounded-[4px] border border-[#DBD7C9]">
                ...
                <div className="text-xs text-[#8B93A0] font-mono">
                  Showing <strong>{filteredLeaderboard.length}</strong> of {leaderboard.length} participants
                </div>
              </div>
          ...
          {activeTab === 'winners1' && (
            <div className="space-y-6">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <button
                    onClick={() => downloadCSV(
                      ...
                      ['Rank','Roll Number','Name','Email','Year','MCQ Score (/ 25)','Qualified for Round 2','Violations'],
                      ['r1_rank','roll_number','name','email','academic_year','mcq_score','mcq_qualified','violations']
                    )}
              ...
              <div className="flex flex-wrap items-center gap-3 bg-white p-3 rounded-[4px] border border-[#DBD7C9]">
                ...
                <div className="text-xs text-[#8B93A0] font-mono">
                  Showing <strong>{filteredWinners1.length}</strong> of {winners1.length} participants
                </div>
              </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[filteredWinners1[1], filteredWinners1[0], filteredWinners1[2]].map((w, i) => (
                    <div key={w?.participant_id} className={`rounded-[6px] p-4 border-2 text-center ${
                      ...
                      <div className="text-[11px] text-[#8B93A0] truncate">{w?.email}</div>
                      <div className="text-xl font-bold font-mono text-[#16233F] mt-2">
                        {w?.mcq_score}<span className="text-sm text-[#8B93A0]"> / 25 pts</span>
                      </div>
                      <div className="text-[10px] text-[#59626F] mt-0.5">Violations: {w?.violations}</div>
                    </div>
                  ))}
                </div>
              )}
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <button
                    onClick={() => downloadCSV(
                      ...
                      ['Rank','Roll Number','Name','Email','Year','MCQ Score (/25)','Debugging Score (/45)','Total Score','Violations'],
                      ['r2_rank','roll_number','name','email','academic_year','mcq_score','coding_score','total_score','violations']
                    )}
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[filteredWinners2[1], filteredWinners2[0], filteredWinners2[2]].map((w, i) => (
                    <div key={w?.participant_id} className={`rounded-[6px] p-4 border-2 text-center ${
                      ...
                      <div className="text-[11px] text-[#8B93A0] truncate">{w?.email}</div>
                      <div className="text-xl font-bold font-mono text-[#16233F] mt-2">
                        {w?.coding_score ?? 0}<span className="text-sm text-[#8B93A0]"> / 45 pts (Debug)</span>
                      </div>
                  ...
                  <tbody className="divide-y divide-[#DBD7C9]">
                    {filteredWinners2.map((e) => (
                      <tr key={e.participant_id} className={`${e.r2_rank <= 3 ? 'bg-[#FFFBEB]' : ''} hover:bg-[#F6F6F2]/70`}>
                        ...
                        <td className="py-2.5 px-3 text-center font-mono">{e.coding_score !== null ? `${e.coding_score} / 45` : '—'}</td>
